...
class ROIPolygon(ROIObject):
    ...
    def __init__(self, x_coords, y_coords, top=None, left=None, bottom=None, right=None):
        self.top = 0 if top is None else top
        self.left = 0 if left is None  else left
        self.bottom = max(y_coords) if bottom is None else bottom
        self.right = max(x_coords) if right is None else right
        self.x_coords = x_coords
        self.y_coords = y_coords
        self.points = list(zip(self.x_coords, self.y_coords))
        self.subpixel = True
    ...
    def _encode_points(self):
        arr = np.array(self.points, dtype=np.int16)
        if self.subpixel:
            arr_sub = arr
        else:
            arr_sub = np.zeros_like(arr)
        arr_nonsub = (arr - [self.left, self.top]).astype(np.int8)
...
class ROIEncoder(ROIFileObject):
    ...
    def write(self):
        ...
        self._write_var('VERSION_OFFSET', 227)  # todo or 226??
    ...
    def _write_roi_polygon(self):
        ...
        self._write_var('BOTTOM', self.roi_obj.bottom)
        self._write_var('RIGHT', self.roi_obj.right)
        self._write_var('N_COORDINATES', len(self.roi_obj.points))
        self._write_var('HEADER2_OFFSET', 64+4*len(self.roi_obj.points))
        self.header2_offset = 64+4*len(self.roi_obj.points)
        self.name_offset = 64+4*len(self.roi_obj.points) + 64
        self.f_obj.seek(64)
        points = np.concatenate((self.roi_obj.x_coords, self.roi_obj.y_coords))
        binary = struct.pack('>' + str(2*len(self.roi_obj.points)) + 'h', *points)
        self.f_obj.write(binary)
        # self.f_obj.seek(65)
        # self.f_obj.write(self.roi_obj._encode_points())
        self._write_var('NAME_OFFSET', self.name_offset)
        self._write_name()

    def _write_var(self, var_name, value, endian='>'):
        ...
        binary = struct.pack(endian + var.type, value)
        self.f_obj.write(binary)
    ...
    def _write_name(self):
        ...
        self.f_obj.write(self.name.encode('utf-8'))
...
class ROIDecoder(ROIFileObject):
    ...
    def _get_roi_polygon(self):
        ...
        x_coords = np.array(coords[:n_coords]) + left
        y_coords = np.array(coords[n_coords:]) + top

        return ROIPolygon(x_coords, y_coords, top, left, bottom, right)
